...
bool imgui_util::back_button(const char *label, ImVec2 size, float rounding = 5)
{
    ...
    bool state = default_rounded_button(label, size, rounding);
    reset_color(3);

    return state;
}

bool imgui_util::confirm_button(const char *label, ImVec2 size, float rounding = 5)
{
    change_button_color(color::green(0.6f));
    change_button_hover_color(color::green(0.3f));
    change_button_clicked_color(color::green(0.8f));
    bool state = default_rounded_button(label, size, rounding);
    reset_color(3);
...
bool imgui_util::default_button(const char *label, ImVec2 size)
{
    return ImGui::Button(label, size);
}

bool imgui_util::rounded_default_color_button(const char *label, ImVec2 size, float rounding)
{
    change_button_color(color::tufts_blue(0.6f));
    change_button_hover_color(color::tufts_blue(0.3f));
    change_button_clicked_color(color::tufts_blue(0.8f));
    bool result = default_rounded_button(label, size, rounding);
    reset_color(3);
    return result;
}
...
bool imgui_util::default_rounded_button(const char *label, const ImVec2 size, const float rounding)
{
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, rounding);
